...
# --------------------------------------------------------------------------
# Process entry-point hygiene: signals and the console
# --------------------------------------------------------------------------


def test_ignore_keyboard_interrupt_takes_the_process_out_of_ctrl_c_s_reach():
    """
    Ctrl+C raises SIGINT in every process of the foreground group, not only the
    one the operator is looking at. A child that dies to it kills the orderly
    shutdown the launcher owns - CORE went before it could pass StopSequencer
    and StopReport on, orphaning its two submodule threads.

    SIG_IGN specifically, not a handler: a Python signal handler runs in the
    main thread between bytecodes and `logging` takes locks, so a handler that
    logged could deadlock against a log call that thread was already making.
    """
...
def test_ignore_keyboard_interrupt_is_harmless_off_the_main_thread():
    """
    `signal.signal()` raises ValueError anywhere but the main thread. Every real
    caller is a process entry point, but a helper that can take a run down when
    called from the wrong place is not a helper.
    """
    failures = []
