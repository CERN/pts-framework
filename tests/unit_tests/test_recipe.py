...
from pypts.recipe import Recipe, StepResult, ResultType, Recipe, Step, IndexedStep, PythonModuleStep, SequenceStep, UserInteractionStep, WaitStep, SerialNumberStep
import uuid
...
def test_recipe_loading():
    '''
    Test that the recipe is loaded correctly by loading a fake recipe and checking
    that the expected pre_run_recipe event is emitted when run.
    '''
    recipe_data = [
    ...
    recipe.run(runtime=mock_runtime)
...
def test_serial_number_step():
    '''
    Test that SerialNumberStep sends the get_serial_number event, stores the serial
    number in runtime.serial_number, and sets the serial_number global variable.
    '''
