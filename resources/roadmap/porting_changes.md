...
| PythonModuleStep, minimal | §1.16 | `action_type: method` only; `module:` = file beside the recipe (`Recipe.base_dir` → `Runtime.base_dir`) or dotted import. Demo: `resources/recipes/python_demo.yml` + `example_tests.py` — PASS/PASS/DONE(2 s)/FAIL(on purpose)/PASS → `Run finished: FAIL (5 steps)` |
