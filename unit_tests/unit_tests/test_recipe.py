...
import pypts
from pypts import StepResult, ResultType, Recipe, Step, PythonModuleStep, SequenceStep, UserInteractionStep, WaitStep
...
def test_serialize_enum():
    assert pypts.UTILITIES.recipe.serialize(Color.RED) == "RED"
    assert pypts.UTILITIES.recipe.serialize(Color.GREEN) == "GREEN"
    assert pypts.UTILITIES.recipe.serialize(Color.BLUE) == "BLUE"



# 3. Test serialization of an object without attributes (e.g., integers, floats, strings)
def test_serialize_basic_types():
    assert pypts.UTILITIES.recipe.serialize(1234) == "1234"
    assert pypts.UTILITIES.recipe.serialize(45.67) == "45.67"
    assert pypts.UTILITIES.recipe.serialize("hello") == "hello"


# 6. Test serialization of None (edge case)
def test_serialize_none():
    assert pypts.UTILITIES.recipe.serialize(None) == "None"
...
def runtime():
    ...
    return pypts.UTILITIES.recipe.Runtime(event_queue, report_queue)


def test_runtime_initialization(runtime):
    """Test if the Runtime class is initialized correctly."""
    assert isinstance(runtime, pypts.UTILITIES.recipe.Runtime)
...
def test_sequence_init_missing_data_raises():
    with pytest.raises(FileNotFoundError):
        pypts.UTILITIES.recipe.Sequence()
...
def test_sequence_init_from_data():
    ...
    with patch("pypts.recipe.Step.build_step", return_value=mock_step):
        sequence = pypts.UTILITIES.recipe.Sequence(sequence_data=sequence_data)
...
def test_sequence_run_executes_steps(mock_evaluate, mock_run_steps):
    ...
    with patch("pypts.recipe.Step.build_step", return_value=MagicMock()):
        seq = pypts.UTILITIES.recipe.Sequence(sequence_data=sequence_data)
