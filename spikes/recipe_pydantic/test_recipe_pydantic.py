...
"""Executable evaluation suite for the isolated Pydantic recipe spike."""
...
def test_normalized_bundled_corpus_passes_as_v2(path):
    legacy = parse_v1(path)
    assert legacy.is_valid, legacy.errors
    normalized = dump_v1(legacy.require_recipe()).replace(
        "recipe_version: 1.0.0", "recipe_version: 2.0.0"
    )
    result = parse_recipe_text(normalized, f"normalized:{path.name}")
    assert result.is_valid, result.errors
    assert parse_recipe_text(dump_recipe(result.require_recipe())).recipe == result.recipe
